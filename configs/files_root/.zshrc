...
# Custom Aliases (Rutas absolutas por seguridad adaptadas a Debian/Kali)
# -----------------------------------------------
...
alias grepc='/usr/bin/grep --colour=always'
...
function extractPorts(){
    ...
    # xclip utilizará automáticamente el $DISPLAY inyectado por Docker
    echo $ports | tr -d '\n' | xclip -sel clip
    echo -e "[*] Ports copied to clipboard\n"
}

# Fix para aplicaciones Java (BurpSuite, Ghidra) en entornos Xwayland
alias xrun="env GDK_BACKEND=x11 _JAVA_AWT_WM_NONREPARENTING=1"

# Excelente función para herramientas que requieran el bus del sistema dentro del contenedor
function sinDB() {
    if [ "$#" -eq 0 ]; then

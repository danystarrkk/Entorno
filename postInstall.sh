...
function installDependencias() {
  ...
  if [ $opt1 == "y" ]; then
    ...
    sudo pacman -S --noconfirm --needed nautilus nmap whatweb arp-scan gobuster ffuf wfuzz burpsuite curl wget netcat openssh python ttf-dejavu ttf-liberation noto-fonts noto-fonts-cjk noto-fonts-emoji noto-fonts-extra ttf-ubuntu-font-family ttf-opensans ttf-roboto adobe-source-han-sans-otc-fonts adobe-source-han-serif-otc-fonts xdg-user-dirs seclists openjdk21-src wl-clipboard thunar aircrack-ng hydra john sqlmap nikto dirb wireshark-cli tcpdump ettercap macchanger reaver bully pixiewps crunch hashcat fcrackzip steghide binwalk foremost sleuthkit volatility recon-ng theharvester dnsrecon dnsenum fierce wafw00f cewl medusa ncrack patator ophcrack samdump2 chntpw metasploit-framework exploitdb searchsploit wpscan commix xsstrike bettercap dsniff dmitry spiderfoot sherlock holehe exiftool radare2 proxychains-ng tor sshuttle mdk4 wifite kismet cowpatty hcxdumptool hcxtools hashcat-utils maskprocessor kwprocessor statsprocessor princeprocessor outguess pdf-parser oletools radare2 strace ltrace binutils
